...
func TestParse(t *testing.T) {
	entries := []struct {
		...
		{
			expr:     "@every 5m",
			expected: &TickSchedule{Every: time.Duration(5) * time.Minute},
		},
		{
			expr: "@every Xm",
...
func TestStandardCronSchedule(t *testing.T) {
	entries := []struct {
		expr     string
		expected Schedule
		err      string
		...
		{
			expr:     "5 * * * *",
			expected: &CronSchedule{1 << seconds.min, 1 << 5, all(hours), all(dom), all(months), all(dow)},
		},
		{
			expr:     "@every 5m",
			expected: &TickSchedule{time.Duration(5) * time.Minute},
		},
		{
			expr: "5 j * * *",
			err:  "Failed to parse int from",
